...
class Owner_Model extends CI_Model{
  ...
  function get_will_list_without_user(){
     $query = $this->db->select('t1.*, t2.*')
      ->from('tbl_will as t1')
      ->join('tbl_personal_info as t2', 't1.will_id = t2.will_id', 'LEFT')
      ->where('t1.will_user_id = ','')
  ...
  public function all_will_no_user_count(){
    $query = $this->db->select('id')
      ->from('tbl_will')
      ->where('will_user_id != ','')
      ->get();
  ...
  public function complete_will_no_user_count(){
    $query = $this->db->select('id')
      ->from('tbl_will')
      ->where('is_will_complete',1)
      ->where('will_user_id != ','')
  ...
  public function incomplete_will_no_user_count(){
    $query = $this->db->select('id')
      ->from('tbl_will')
      ->where('is_will_complete',0)
      ->where('will_user_id != ','')
